...
const Dashboard = () => {
  ...
  // Voucher List Views State (for text click)
  ...
  // Sample data for voucher lists (enhanced for Edit/View/Delete)
  const [salesListData, setSalesListData] = useState([
    { id: 1, billDate: '2024-01-15', billSeries: 'INV', billNo: 'INV-001', billType: 'Credit', partyCode: 'ABC001', partyName: 'ABC Corp', branchName: 'Main Branch', amount: '15000.00', loadSeries: 'LS-01', loadNumber: 'LD-123', status: 'Paid', items: [], summary: {} },
    { id: 2, billDate: '2024-01-20', billSeries: 'INV', billNo: 'INV-002', billType: 'Cash', partyCode: 'XYZ002', partyName: 'XYZ Ltd', branchName: 'North Branch', amount: '25000.00', loadSeries: 'LS-02', loadNumber: 'LD-456', status: 'Pending', items: [], summary: {} },
  ]);
  const [purchaseListData, setPurchaseListData] = useState([
    { id: 1, invoiceDate: '2024-01-10', billSeries: 'PO', invoiceNumber: 'PO-001', billType: 'Credit', partyCode: 'SUP001', partyName: 'Supplier A', branchName: 'Warehouse', amount: '50000.00', loadSeries: 'PS-01', loadNumber: 'PL-789', status: 'Received', items: [], summary: {} },
    { id: 2, invoiceDate: '2024-01-18', billSeries: 'PO', invoiceNumber: 'PO-002', billType: 'Credit', partyCode: 'SUP002', partyName: 'Supplier B', branchName: 'Factory', amount: '35000.00', loadSeries: 'PS-02', loadNumber: 'PL-012', status: 'Pending', items: [], summary: {} },
  ]);
  const [creditNoteListData, setCreditNoteListData] = useState([
    { id: 1, vDate: '2024-01-05', billSeries: 'CN', vNo: 'CN-001', billType: 'Credit', partyCode: 'ABC001', partyName: 'ABC Corp', branchName: 'Main Branch', amount: '5000.00', loadSeries: 'CS-01', loadNumber: 'CL-345', status: 'Approved', items: [], summary: {} },
    { id: 2, vDate: '2024-01-12', billSeries: 'CN', vNo: 'CN-002', billType: 'Credit', partyCode: 'XYZ002', partyName: 'XYZ Ltd', branchName: 'North Branch', amount: '3000.00', loadSeries: 'CS-02', loadNumber: 'CL-678', status: 'Pending', items: [], summary: {} },
  ]);
  const [debitNoteListData, setDebitNoteListData] = useState([
    { id: 1, vDate: '2024-01-08', billSeries: 'DN', vNo: 'DN-001', billType: 'Debit', partyCode: 'SUP001', partyName: 'Supplier A', branchName: 'Warehouse', amount: '2000.00', loadSeries: 'DS-01', loadNumber: 'DL-901', status: 'Approved', items: [], summary: {} },
    { id: 2, vDate: '2024-01-15', billSeries: 'DN', vNo: 'DN-002', billType: 'Debit', partyCode: 'SUP002', partyName: 'Supplier B', branchName: 'Factory', amount: '4500.00', loadSeries: 'DS-02', loadNumber: 'DL-234', status: 'Pending', items: [], summary: {} },
  ]);
  ...
  const calcInvoiceSummary = (currentItems) => {
    ...
    setInvoiceSummary({ 
      gross, tpr, scheme, bottom: taxable, star, cd, gst, net: Math.round(net * 100) / 100 
    });
  };
  ...
  const existingBatches = batches.filter(b => b.productId === purchaseItems[currentBatchRow]?.productId);
  ...
  const calcCreditNoteSummary = (currentItems) => {
    let grossAmt = 0, schemeAmt = 0, tprAmt = 0, cashDisc = 0, gstAmt = 0;
    currentItems.forEach(item => {
  ...
  const getFilteredAccounts = () => accounts.filter(account =>
  ...
  const getFilteredOtherAccounts = () => otherAccounts.filter(account =>
  ...
  const getFilteredGST = () => gstList.filter(gst =>
    gst.code?.toLowerCase().includes(filters.gst.toLowerCase())
  );

  const getFilteredSalesmen = () => salesmen.filter(salesman =>
  ...
  // Enhanced Voucher Action Handlers
  const handleEditVoucher = (type, item) => {
    console.log(`Editing ${type}:`, item);
    // For now, populate the form with the selected item data
    if (type === 'Sales') {
      setInvoiceFormData({
  ...
  // Helper function to render voucher list view - MODIFIED with Edit/View/Delete buttons
  const renderVoucherList = (title, data, columns) => {
    // Define the 12 standard columns for all voucher modules
    ...
    // Transform data to match the 12-column structure
    ...
    // Determine the type for edit/view/delete
    const getType = () => {
      ...
      <div className="master-section grid-section">
        <div className="grid-header">
          ...
          <div className="table-controls">
            <button className="btn-excel" onClick={() => exportToExcel(transformedData, title, standardColumns.filter(c => c.key !== 'actions'))}>
              📊 Export Excel
            </button>
            <button className="btn-pdf" onClick={() => exportToPDF(transformedData, title, standardColumns.filter(c => c.key !== 'actions'))}>
              📄 Export PDF
            </button>
          </div>
        </div>
              ...
              <thead>
                <tr>
                  {standardColumns.map(col => (
                    <th key={col.key} style={col.key === 'actions' ? { textAlign: 'center', width: '120px' } : {}}>
                      {col.label}
                    </th>
                  ))}
                </tr>
  ...
  useEffect(() => {
    if (activeSubMenu === 'Sales' && openFormFor === 'Sales' && invoiceItems.length === 0) {
      addInvoiceItem();
    }
    if (activeSubMenu === 'Purchase' && openFormFor === 'Purchase' && purchaseItems.length === 0) {
      addPurchaseItem();
    }
    if (activeSubMenu === 'Credit Note' && openFormFor === 'Credit Note' && creditNoteItems.length === 0) {
      addCreditNoteItem();
    }
    if (activeSubMenu === 'Debit Note' && openFormFor === 'Debit Note' && debitNoteItems.length === 0) {
      addDebitNoteItem();
    }
  }, [activeSubMenu, openFormFor, invoiceItems.length, addInvoiceItem, purchaseItems.length, addPurchaseItem, creditNoteItems.length, addCreditNoteItem, debitNoteItems.length, addDebitNoteItem]);
  ...
  return (
    <div className="dashboard-container">
        ...
        <nav className="nav-menu">
          {Object.entries(menuItems).map(([key, menu]) => (
            <div key={key} className="nav-item">
              <div className={`nav-header ${activeMenu === key ? 'active' : ''}`} onClick={() => setActiveMenu(activeMenu === key ? null : key)}>
      ...
      <main className="main-content">
        ...
        <div className="content-body">
          ...
          {showViewModal && viewData && (
            <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }} onClick={(e) => { if (e.target === e.currentTarget) setShowViewModal(false); }}>
              <div style={{ background: 'white', borderRadius: '12px', width: '80%', maxWidth: '900px', maxHeight: '80vh', overflow: 'auto', padding: '24px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #e5e7eb', paddingBottom: '12px' }}>
                  <h2 style={{ margin: 0, fontSize: '20px', fontWeight: '600' }}>{viewType} Details - {viewData.billNo || viewData.invoiceNumber || viewData.vNo}</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
                  <div><strong>Date:</strong> {viewData.billDate || viewData.invoiceDate || viewData.vDate || '-'}</div>
                  <div><strong>Bill Series:</strong> {viewData.billSeries || '-'}</div>
                  <div><strong>Bill Number:</strong> {viewData.billNo || viewData.invoiceNumber || viewData.vNo || '-'}</div>
                  <div><strong>Bill Type:</strong> {viewData.billType || '-'}</div>
                  <div><strong>Party Name:</strong> {viewData.party || viewData.supplier || viewData.partyName || '-'}</div>
                  <div><strong>Amount:</strong> ₹{parseFloat(viewData.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
                  <div><strong>Status:</strong> <span className={`status-badge ${viewData.status?.toLowerCase() === 'paid' || viewData.status?.toLowerCase() === 'approved' ? 'status-success' : 'status-warning'}`}>{viewData.status || '-'}</span></div>
                </div>
                      ...
                      <tbody>
                        {viewData.items.map((item, idx) => (
                          <tr key={idx} style={{ borderBottom: '1px solid #e5e7eb' }}>
                            <td style={{ padding: '8px' }}>{item.product || item.itemName || '-'}</td>
                            <td style={{ padding: '8px', textAlign: 'right' }}>{item.qty || item.quantity || '-'}</td>
                            <td style={{ padding: '8px', textAlign: 'right' }}>{item.rate || '-'}</td>
                            <td style={{ padding: '8px', textAlign: 'right' }}>{item.amount || item.grossAmt || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'flex-end' }}>
                  <button onClick={() => setShowViewModal(false)} style={{ padding: '8px 20px', background: '#3b82f6', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Close</button>
                </div>
              </div>
            </div>
          )}
          ...
          {/* VOUCHERS SECTION - FIXED with Edit/View/Delete */}
          {/* Sales - Show List when text clicked, Form when + icon clicked */}
          {activeSubMenu === 'Sales' && showSalesList && openFormFor !== 'Sales' && (
            renderVoucherList('Sales', salesListData, [])
          )}

          {/* Sales Form */}
          {activeSubMenu === 'Sales' && openFormFor === 'Sales' && (
            <div className="master-section erp-master-form sales-invoice">
              <div className="erp-header">
                <div><h2 className="erp-title">Sales Invoice</h2></div>
              </div>
              <div className="form-section">
                      ...
                      <tbody>
                        {invoiceItems.map((item, index) => (
                          <tr key={item.id} className={index === activeRow ? 'active-row' : ''}>
                                  ...
                                  <div className="product-dropdown">
                                    <div className="dropdown-list">
                                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                                        ...
                                        <tbody>
                                          {products
                                            .filter(p => p.name?.toLowerCase().includes(productListFilter.toLowerCase()) || p.code?.toLowerCase().includes(productListFilter.toLowerCase()))
                                            .map(product => (
                                              <tr key={product.id} onClick={() => handleProductSelect(currentProductIndex, product)} style={{ cursor: 'pointer' }}>
                                                <td style={{ padding: '6px' }}>{product.code}</td>
                                                <td style={{ padding: '6px' }}>{product.name}</td>
                                                <td style={{ padding: '6px', textAlign: 'right' }}>{product.Rate_Per_Unit}</td>
                                              </tr>
                                            ))}
                                        </tbody>
                                      </table>
                            ...
                            <td>
                              <select className="erp-select" data-field="units" data-index={index} value={item.units} onChange={(e) => updateInvoiceItem(index, 'units', e.target.value)} onKeyDown={(e) => handleInvoiceKeyDown(e, index, 'units')} style={{ width: '100%' }}>
                                <option value="PCS">PCS</option>
                                <option value="BOX">BOX</option>
                                <option value="INBOX">INBOX</option>
                                <option value="KG">KG</option>
              ...
              <div className="form-actions">
                <button className="btn-add-invoice" onClick={saveInvoice}>💾 Add Invoice</button>
              </div>
            </div>
          )}

          {/* Purchase - Show List when text clicked, Form when + icon clicked */}
          {activeSubMenu === 'Purchase' && showPurchaseList && openFormFor !== 'Purchase' && (
          ...
          {activeSubMenu === 'Purchase' && openFormFor === 'Purchase' && (
            <div className="master-section erp-master-form purchase-invoice">
              <div className="erp-header"><div><h2 className="erp-title">Add Purchase</h2></div></div>
              <div className="form-section">
                <div className="purchase-header-grid">
                  <div className="labeled-input"><label>Date</label><input type="date" name="invoiceDate" className="erp-input" value={purchaseFormData.invoiceDate} onChange={handlePurchaseInputChange} /></div>
                  <div className="labeled-input"><label>SUPPLIER *</label><select name="supplier" className="erp-select" value={purchaseFormData.supplier} onChange={handlePurchaseInputChange}><option value="">Select Supplier</option>{accounts.map(acc => <option key={acc.id} value={acc.accountName}>{acc.accountName}</option>)}</select></div>
                  <div className="labeled-input"><label>COMPANY *</label><select name="company" className="erp-select" value={purchaseFormData.company} onChange={handlePurchaseInputChange}><option value="">Select Company</option>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div>
                  <div className="labeled-input"><label>STORAGE LOCATION/GODOWN *</label><select name="storageLocation" className="erp-select" value={purchaseFormData.storageLocation} onChange={handlePurchaseInputChange}><option value="">Select Godown</option>{godowns.map(g => <option key={g.id} value={g.name}>{g.name}</option>)}</select></div>
                  <div className="labeled-input"><label>ENTER INVOICE NUMBER *</label><input type="text" name="invoiceNumber" className="erp-input" placeholder="Enter Invoice Number" value={purchaseFormData.invoiceNumber} onChange={handlePurchaseInputChange} required /></div>
                  <div className="labeled-input narration-field" style={{ gridColumn: 'span 2' }}><label>Narration</label><textarea name="narration" className="erp-textarea" rows="2" style={{ width: '100%', minHeight: '60px', resize: 'vertical' }} value={invoiceFormData.narration} onChange={handleInvoiceInputChange} /></div>
                </div>
              </div>
              <div className="form-section">
                      ...
                      <tbody>
                        {purchaseItems.map((item, index) => (
                          <tr key={item.id} className={index === purchaseActiveRow ? 'active-row' : ''}>
                            ...
                            <td>
                              <input data-purchase-product-index={index} className="erp-input" placeholder="Click to select product" value={item.product} onChange={(e) => { updatePurchaseItem(index, 'product', e.target.value); setPurchaseProductListFilter(e.target.value); setCurrentPurchaseProductIndex(index); setShowPurchaseProductList(true); }} onFocus={() => { setPurchaseActiveRow(index); if (item.product) { setPurchaseProductListFilter(item.product); setCurrentPurchaseProductIndex(index); setShowPurchaseProductList(true); } }} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'product')} style={{ width: '100%', cursor: 'pointer' }} />
                              {showPurchaseProductList && currentPurchaseProductIndex === index && (
                                <div className="product-dropdown-overlay">
                                  <div className="product-dropdown">
                                    <div className="dropdown-list">
                                      {products.filter(p => p.name?.toLowerCase().startsWith(purchaseProductListFilter.toLowerCase()) || p.code?.toLowerCase().startsWith(purchaseProductListFilter.toLowerCase())).slice(0, 10).map(product => (
                                        <div key={product.id} className="dropdown-item" onClick={() => handlePurchaseProductSelect(currentPurchaseProductIndex, product)}><strong>{product.code}</strong> - {product.name}</div>
                                      ))}
                                    </div>
                                  </div>
                                </div>
                            ...
                            <td><select data-purchase-field="unitId" data-purchase-index={index} className="erp-select" value={item.unitId} onChange={(e) => updatePurchaseItem(index, 'unitId', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'unitId')} style={{ width: '100%' }}><option value="PCS">PCS</option><option value="BOX">BOX</option><option value="INBOX">INBOX</option><option value="KG">KG</option><option value="LTR">LTR</option></select></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="quantity" data-purchase-index={index} placeholder="Qty" value={item.quantity} onChange={(e) => updatePurchaseItem(index, 'quantity', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'quantity')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="free" data-purchase-index={index} placeholder="Free" value={item.free} onChange={(e) => updatePurchaseItem(index, 'free', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'free')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="mrp" data-purchase-index={index} placeholder="MRP" value={item.mrp} onChange={(e) => updatePurchaseItem(index, 'mrp', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'mrp')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="purRate" data-purchase-index={index} placeholder="Pur Rate" value={item.purRate} onChange={(e) => updatePurchaseItem(index, 'purRate', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'purRate')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="disc1" data-purchase-index={index} placeholder="Disc 1 %" value={item.disc1} onChange={(e) => updatePurchaseItem(index, 'disc1', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'disc1')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="disc2" data-purchase-index={index} placeholder="Disc 2 %" value={item.disc2} onChange={(e) => updatePurchaseItem(index, 'disc2', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'disc2')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="disc3" data-purchase-index={index} placeholder="Disc 3 %" value={item.disc3} onChange={(e) => updatePurchaseItem(index, 'disc3', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'disc3')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="taxable" data-purchase-index={index} placeholder="Taxable" value={item.taxable} onChange={(e) => updatePurchaseItem(index, 'taxable', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'taxable')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="tax" data-purchase-index={index} placeholder="Tax %" value={item.tax} onChange={(e) => updatePurchaseItem(index, 'tax', e.target.value)} onKeyDown={(e) => handlePurchaseKeyDown(e, index, 'tax')} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="cgst" data-purchase-index={index} placeholder="CGST" value={item.cgst} onChange={(e) => updatePurchaseItem(index, 'cgst', e.target.value)} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric" type="number" data-purchase-field="sgst" data-purchase-index={index} placeholder="SGST" value={item.sgst} onChange={(e) => updatePurchaseItem(index, 'sgst', e.target.value)} style={{ width: '100%' }} /></td>
                            <td><input className="erp-input numeric font-bold" type="number" data-purchase-field="amount" data-purchase-index={index} placeholder="Amount" value={item.amount} onChange={(e) => updatePurchaseItem(index, 'amount', e.target.value)} style={{ width: '100%', fontWeight: 'bold' }} /></td>
                            <td style={{ position: 'sticky', right: 0, backgroundColor: index === purchaseActiveRow ? '#eff6ff' : 'white', zIndex: 5 }}><button className="btn-delete" onClick={() => deletePurchaseItem(index)}>🗑 Delete</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              <div className="form-section">
                ...
                <div className="summary-bar">
                  ...
                  <div>Net Amount: <strong>₹{purchaseFormData.netAmt || '0.00'}</strong></div>
                </div>
              </div>
              <div className="form-actions"><button className="btn-add-invoice" onClick={savePurchase}>💾 Add Invoice</button></div>
            </div>
          )}

          {/* Credit Note - Show List when text clicked, Form when + icon clicked */}
          {activeSubMenu === 'Credit Note' && showCreditNoteList && openFormFor !== 'Credit Note' && (
          ...
          {activeSubMenu === 'Credit Note' && openFormFor === 'Credit Note' && (
            <div className="master-section erp-master-form credit-note">
              <div className="erp-header"><div><h2 className="erp-title">Credit Note</h2></div></div>
              {showCreditProductList && currentCreditProductIndex >= 0 && (
                <div className="product-dropdown-overlay">
                  <div className="product-dropdown">
                    <div className="dropdown-header"><input type="text" placeholder="Search products..." value={creditProductListFilter} onChange={(e) => setCreditProductListFilter(e.target.value)} autoFocus /><button onClick={() => setShowCreditProductList(false)}>✕</button></div>
                    <div className="dropdown-list">{products.filter(p => p.name.toLowerCase().includes(creditProductListFilter.toLowerCase()) || p.code.toLowerCase().includes(creditProductListFilter.toLowerCase())).map(product => (<div key={product.id} className="dropdown-item" onClick={() => handleCreditProductSelect(currentCreditProductIndex, product)}><strong>{product.code}</strong> - {product.name}</div>))}</div>
                  </div>
                </div>
              )}
              <div className="form-section">
                ...
                <div className="invoice-header-grid">
                  ...
                  <div className="labeled-input"><label>Party *</label><select name="party" className="erp-select" value={creditNoteFormData.party} onChange={handleCreditNoteInputChange}><option value="">Select Party</option>{accounts.map(a => <option key={a.id} value={a.accountName}>{a.accountName}</option>)}</select></div>
                  <div className="labeled-input"><label>Company</label><select name="company" className="erp-select" value={creditNoteFormData.company} onChange={handleCreditNoteInputChange}><option value="">Select Company</option>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div>
              ...
              <div className="form-section">
                ...
                <div className="product-entry-container" style={{ overflowX: 'auto', maxHeight: '300px' }}>
                  <table className="product-entry-table">
                    <thead>
                      <tr><th>Sr</th><th>Item Code</th><th>Item Name</th><th>Qty</th><th>Rate</th><th>Amount</th><th>Action</th></tr>
                    </thead>
                    <tbody>
                      {creditNoteItems.map((item, index) => (
                        <tr key={item.id}>
                          <td>{item.sr}</td>
                          <td><input className="erp-input" value={item.itemCode} onChange={(e) => updateCreditNoteItem(index, 'itemCode', e.target.value)} onClick={() => setCurrentCreditProductIndex(index)} placeholder="Select" /></td>
                          <td><input className="erp-input" value={item.itemName} onChange={(e) => updateCreditNoteItem(index, 'itemName', e.target.value)} /></td>
                          <td><input className="erp-input numeric" type="number" value={item.qty} onChange={(e) => updateCreditNoteItem(index, 'qty', e.target.value)} /></td>
                          <td><input className="erp-input numeric" type="number" value={item.rate} onChange={(e) => updateCreditNoteItem(index, 'rate', e.target.value)} /></td>
                          <td><input className="erp-input numeric" value={item.grossAmt} readOnly /></td>
                          <td><button className="btn-delete" onClick={() => deleteCreditNoteItem(index)}>Delete</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="form-actions"><button className="btn-add-invoice" onClick={saveCreditNote}>💾 Save Credit Note</button></div>
            </div>
          )}

          {/* Debit Note - Show List when text clicked, Form when + icon clicked */}
          {activeSubMenu === 'Debit Note' && showDebitNoteList && openFormFor !== 'Debit Note' && (
          ...
          {activeSubMenu === 'Debit Note' && openFormFor === 'Debit Note' && (
            <div className="master-section erp-master-form debit-note">
              <div className="erp-header"><div><h2 className="erp-title">Debit Note</h2></div></div>
              {showDebitProductList && currentDebitProductIndex >= 0 && (
                <div className="product-dropdown-overlay">
                  <div className="product-dropdown">
                    <div className="dropdown-header"><input type="text" placeholder="Search products..." value={debitProductListFilter} onChange={(e) => setDebitProductListFilter(e.target.value)} autoFocus /><button onClick={() => setShowDebitProductList(false)}>✕</button></div>
                    <div className="dropdown-list">{products.filter(p => p.name.toLowerCase().includes(debitProductListFilter.toLowerCase()) || p.code.toLowerCase().includes(debitProductListFilter.toLowerCase())).map(product => (<div key={product.id} className="dropdown-item" onClick={() => handleDebitProductSelect(currentDebitProductIndex, product)}><strong>{product.code}</strong> - {product.name}</div>))}</div>
                  </div>
                </div>
              )}
              <div className="form-section">
                ...
                <div className="invoice-header-grid">
                  ...
                  <div className="labeled-input"><label>Supplier</label><select name="supplier" className="erp-select" value={debitNoteFormData.supplier} onChange={handleDebitNoteInputChange}><option value="">Select Supplier</option>{accounts.map(a => <option key={a.id} value={a.accountName}>{a.accountName}</option>)}</select></div>
                  <div className="labeled-input"><label>Company</label><select name="company" className="erp-select" value={debitNoteFormData.company} onChange={handleDebitNoteInputChange}><option value="">Select Company</option>{companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></div>
                  ...
                  <table className="product-entry-table">
                    ...
                    <tbody>
                      {debitNoteItems.map((item, index) => (
                        <tr key={item.id}>
                          <td>{item.sr}</td>
                          <td><input className="erp-input" value={item.itemCode} onChange={(e) => updateDebitNoteItem(index, 'itemCode', e.target.value)} onClick={() => setCurrentDebitProductIndex(index)} placeholder="Select" /></td>
                          <td><input className="erp-input" value={item.itemName} onChange={(e) => updateDebitNoteItem(index, 'itemName', e.target.value)} /></td>
                          <td><input className="erp-input numeric" type="number" value={item.qty} onChange={(e) => updateDebitNoteItem(index, 'qty', e.target.value)} /></td>
                          <td><input className="erp-input numeric" type="number" value={item.rate} onChange={(e) => updateDebitNoteItem(index, 'rate', e.target.value)} /></td>
                          <td><input className="erp-input numeric" value={item.grossAmt} readOnly /></td>
                          <td><button className="btn-delete" onClick={() => deleteDebitNoteItem(index)}>Delete</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="form-actions"><button className="btn-add-invoice" onClick={saveDebitNote}>💾 Save Debit Note</button></div>
            </div>
          )}
            ...
            <div className="master-section">
              ...
              {areaToPartyCompany && areaToPartyData.length > 0 && (
                <>
                  ...
                  <div className="data-table-container">
                    <table className="data-table">
                      <thead><tr><th>Account Code</th><th>Account Name</th><th>Area Name</th><th>Status</th></tr></thead>
                      <tbody>
                        {areaToPartyData.map((row, rowIndex) => (
                          <tr key={row.id}>
                            <td>{row.accountCode}</td>
                            <td>{row.accountName}</td>
                            <td className={`editable-cell ${areaToPartyEditingCell === `areaName_${rowIndex}` ? 'editing' : ''}`} onClick={() => { setAreaToPartyEditingRow(rowIndex); setAreaToPartyEditingCell(`areaName_${rowIndex}`); setShowAreaToPartyDropdown(true); }} tabIndex={0}>
                              {row.areaName ? row.areaName : <span className="placeholder">Click to select area...</span>}
                              {showAreaToPartyDropdown && areaToPartyEditingRow === rowIndex && (<div className="dropdown-overlay"><div className="dropdown-filter"><input type="text" placeholder="Search Area..." value={areaToPartyFilter} onChange={(e) => setAreaToPartyFilter(e.target.value)} autoFocus /></div><div className="dropdown-list">{areas.filter(area => area.name.toLowerCase().includes(areaToPartyFilter.toLowerCase())).map(area => (<div key={area.id} className="dropdown-item" onClick={() => handleAreaToPartySelect(rowIndex, area)}><strong>{area.code}</strong> - {area.name}</div>))}</div></div>)}
                            </td>
                            <td><span className={`status-badge ${row.areaName ? 'complete' : 'pending'}`}>{row.areaName ? '✅ Complete' : '⏳ Pending'}</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {/* Batch Modal */}
      {showBatchModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 999999 }} onClick={(e) => { if (e.target === e.currentTarget) setShowBatchModal(false); }}>
          <div style={{ width: '800px', background: 'white', borderRadius: '10px', padding: '20px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', borderBottom: '1px solid #e5e7eb', paddingBottom: '10px' }}>
              <button type="button" onClick={() => setBatchMode('existing')} style={{ padding: '8px 20px', background: batchMode === 'existing' ? '#2563eb' : '#f3f4f6', color: batchMode === 'existing' ? 'white' : '#374151', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Existing Batch</button>
              <button type="button" onClick={() => setBatchMode('new')} style={{ padding: '8px 20px', background: batchMode === 'new' ? '#2563eb' : '#f3f4f6', color: batchMode === 'new' ? 'white' : '#374151', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>New Batch</button>
            </div>
            {batchMode === 'existing' && (
              <>
                <div style={{ border: '1px solid #e5e7eb', borderRadius: '8px', maxHeight: '350px', overflowY: 'auto', marginBottom: '20px' }}>
                  {existingBatches.length === 0 && <div style={{ padding: '40px', textAlign: 'center', color: '#6b7280' }}>No Existing Batch Found</div>}
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    <thead><tr style={{ background: '#f8fafc', position: 'sticky', top: 0 }}><th>Sl</th><th>Batch</th><th>MRP</th><th>Expiry Date</th><th>Stock</th></tr></thead>
                    <tbody>
                      {existingBatches.map((batch, idx) => (
                        <tr key={idx} onClick={() => { const updatedItems = [...purchaseItems]; updatedItems[currentBatchRow] = { ...updatedItems[currentBatchRow], batchNo: batch.batchNo, mfgDate: batch.mfgDate, expDate: batch.expDate, mrp: batch.mrp, purRate: batch.purchaseRate, salesRate: batch.salesRate, quantity: updatedItems[currentBatchRow]?.quantity || 1, stockQty: batch.stockQty }; setPurchaseItems(updatedItems); setShowBatchModal(false); }} style={{ cursor: 'pointer' }}>
                          <td>{idx + 1}</td>
                          <td><span style={{ fontWeight: '500', color: '#2563eb' }}>{batch.batchNo}</span></td>
                          <td>₹{parseFloat(batch.mrp || 0).toFixed(2)}</td>
                          <td>{batch.expDate !== '01/01/1900' && batch.expDate !== '1900-01-01' ? batch.expDate : '-'}</td>
                          <td>{parseFloat(batch.stockQty).toFixed(0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}><button onClick={() => setShowBatchModal(false)} style={{ padding: '8px 20px', background: '#6b7280', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Cancel</button></div>
              </>
            )}
            {batchMode === 'new' && (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
                  <div className="labeled-input"><label>Batch No *</label><input type="text" placeholder="Enter Batch Number" value={batchForm.batchNo} onChange={(e) => setBatchForm({ ...batchForm, batchNo: e.target.value })} /></div>
                  <div className="labeled-input"><label>Expiry Date</label><input type="date" value={batchForm.expDate} onChange={(e) => setBatchForm({ ...batchForm, expDate: e.target.value })} /></div>
                  <div className="labeled-input"><label>MRP *</label><input type="number" placeholder="Enter MRP" value={batchForm.mrp} onChange={(e) => setBatchForm({ ...batchForm, mrp: e.target.value })} /></div>
                  <div className="labeled-input"><label>Purchase Rate</label><input type="number" placeholder="Enter Purchase Rate" value={batchForm.purchaseRate} onChange={(e) => setBatchForm({ ...batchForm, purchaseRate: e.target.value })} /></div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
                  <button onClick={() => setShowBatchModal(false)} style={{ padding: '10px 24px', background: '#9ca3af', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Cancel</button>
                  <button onClick={saveBatchDetails} style={{ padding: '10px 24px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>Save Batch</button>
                </div>
              </>
            )}
          </div>
